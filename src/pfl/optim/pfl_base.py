...
class PartialPFLBase(FedBase):
    ...
    def __init__(self, train_fed_loader, available_clients, clients_to_cache, server_model, client_model,
        ...
        super().__init__(
            ...
            save_client_params_to_disk, stateless_clients, args=kwargs.get('args', None)
        )
    ...
    def update_local_model_and_get_client_grad(self):
        ...
        server_param_grad = OrderedDict(
            (k, old_server_params[k] - new_server_params[k]) for k in old_server_params.keys())
    ...
    def get_client_l2_penalty(self):
        if self.local_var_l2_reg <= 1e-10:
            ...
            client_params = self.combined_model.client_parameters()
            return self.local_var_l2_reg * sum(
                torch.norm(v.reshape(-1) - v1.reshape(-1)) ** 2 for (v, v1) in zip(client_params, self.prox_center))
