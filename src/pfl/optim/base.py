...
class FedBase:
    ...
    def __init__(self, train_fed_loader, available_clients, clients_to_cache, server_model, client_model,
                 ...
                 save_dir, seed, save_client_params_to_disk=False, stateless_clients=False, args=None):
        self.args = args
        ...
        if self.client_model is not None:
            ...
            if not save_client_params_to_disk:  # maintain {client_id -> client_state_dict} mapping for `clients_to_cache`
                self.saved_client_params = {client_id: copy.deepcopy(start_params) for client_id in
                                            self.clients_to_cache}
            else:  # save client_state_dict to disk;
                for client_id in self.clients_to_cache:
                    client_fn = self.get_client_fn(client_id)
                    if not os.path.exists(os.path.dirname(client_fn)):
                        os.makedirs(os.path.dirname(client_fn), exist_ok=True)
        ...
        self.client_indices = {v: i for i, v in enumerate(available_clients)}
    ...
    def run_one_round(self, num_clients_per_round, num_local_epochs, client_optimizer, client_optimizer_args):
        ...
        for i, client_id in enumerate(sampled_clients):
            ...
            client_loader = self.train_fed_loader.get_client_dataloader(client_id, self.client_indices[client_id])
    ...
    def test_all_clients(self, test_fed_loader, max_num_clients=5000, return_all_metrics=False):
        ...
        for client_id in list_of_clients:
            ...
            client_dataloader = test_fed_loader.get_client_dataloader(client_id, self.client_indices[client_id])
            client_size, metrics_for_client = pfl.metrics.compute_metrics_for_client(
                self.combined_model, client_dataloader, self.metrics_fn, temperature=self.temperature
            )
            ...
            if collected_metrics is not None:
                ...
                collected_metrics = OrderedDict(
                    (metric_name, [metric_val]) for (metric_name, metric_val) in metrics_for_client.items())
